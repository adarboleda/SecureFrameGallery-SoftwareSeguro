...
export default function AlbumWorkspace({ params }: { params: Promise<{ id: string }> }) {
  ...
  const statusLabel = (s: string) => {
    if (s === "clean") return { label: "Limpio", color: "bg-green-500", text: "text-green-700" };
    if (s === "quarantined") return { label: "En cuarentena", color: "bg-red-500", text: "text-red-700" };
    return { label: "Procesando", color: "bg-yellow-400", text: "text-yellow-700" };
  };
    ...
    <div className="bg-background text-on-background min-h-screen pb-24 md:pb-0 pt-20 antialiased">
      ...
      <header className="fixed top-0 w-full z-50 bg-white/90 backdrop-blur-xl shadow-[0_4px_20px_-5px_rgba(0,0,0,0.05)]">
        <div className="flex items-center justify-between px-6 py-4 w-full max-w-7xl mx-auto">
          ...
          <h1 className="text-[#E60023] font-bold text-2xl tracking-tighter">SecureFrame</h1>
          <button
      ...
      <main className="px-6 mx-auto max-w-7xl pt-6">
        ...
        <section className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <h2 className="font-bold text-2xl text-on-background mb-1">Espacio de Trabajo del Álbum</h2>
            <p className="text-secondary text-base">Tus archivos multimedia subidos y su estado de análisis.</p>
        ...
        {uploadMessage && (
          <div className={`mb-6 flex items-start gap-3 px-5 py-4 rounded-2xl text-sm ${
            uploadMessage.ok
        ...
        <section>
          {loading ? (
            <div className="py-24 flex flex-col items-center gap-4 text-secondary">
              <span className="material-symbols-outlined animate-spin text-5xl text-[#E60023]">refresh</span>
            ...
            <div className="py-24 flex flex-col items-center gap-5 border-2 border-dashed border-outline-variant rounded-3xl text-on-surface-variant">
              <span className="material-symbols-outlined text-7xl opacity-20">cloud_upload</span>
              <div className="text-center">
                <p className="font-bold text-xl mb-2">Aún no hay archivos subidos</p>
                <p className="text-sm opacity-70">Sube imágenes (JPG, PNG) o documentos (PDF) a este álbum.</p>
              </div>
              <button
                onClick={() => fileInputRef.current?.click()}
                disabled={uploading || !userId}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
              {files.map((file) => {
                ...
                return (
                  <div key={file.id} className="group cursor-pointer" onClick={() => setLightbox(file)}>
                    <div className="rounded-2xl overflow-hidden relative shadow-[0_4px_14px_rgba(0,0,0,0.08)] hover:shadow-[0_8px_28px_rgba(0,0,0,0.12)] transition-all duration-300 bg-surface-container aspect-square">
                      {file.type === "pdf" ? (
                        <div className="w-full h-full flex flex-col items-center justify-center gap-2 bg-red-50">
                          ...
                          <span className="text-xs text-red-700 font-medium">Toca para abrir PDF</span>
                        </div>
                      ) : (
                        <img
                          src={file.url}
                    ...
                    <div className="mt-2 px-1 flex items-center gap-1.5">
                      <span className={`w-2 h-2 rounded-full shrink-0 ${st.color}`}></span>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 w-full px-4 pb-5 md:hidden">
        <div className="flex justify-around items-center max-w-md mx-auto h-16 bg-white/95 backdrop-blur-lg rounded-full shadow-[0_8px_30px_-10px_rgba(0,0,0,0.12)] border border-zinc-100">
          ...
          <button onClick={handleLogout} className="text-zinc-400 hover:bg-zinc-100 rounded-full p-3 flex flex-col items-center cursor-pointer">
      ...
      {lightbox && (
        ...
        >
          ...
          <div onClick={(e) => e.stopPropagation()} className="max-w-5xl w-full max-h-[92vh]">
            {lightbox.type === "pdf" ? (
              <iframe
